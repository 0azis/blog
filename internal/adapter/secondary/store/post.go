...
func (p post) GetPostsByUser(userID int) ([]*domain.Post, error) {
	...
	rows, err := p.db.Query(`select posts.id, posts.title, posts.preview, posts.date, posts.content, users.id, users.username, users.name, users.avatar, count(views.user_id), count(comments.post_id) from posts inner join users on posts.user_id = users.id left join views on posts.id = views.post_id left join comments on posts.id = comments.post_id where public = 1 and posts.user_id = ? group by posts.id`, userID)
	if err != nil {
		return posts, err
	}
...
func (p post) GetByTag(tag string) ([]*domain.Post, error) {
	var posts []*domain.Post
	rows, err := p.db.Query(`select posts.id, posts.title, posts.preview, posts.date, posts.content, users.id, users.username, users.name, users.avatar from posts inner join users on posts.user_id = users.id left join tags on posts.id = tags.post_id where tags.tag = ?`, tag)
	if err != nil {
		return posts, err
	}
	...
	for rows.Next() {
		var post domain.Post
		err = rows.Scan(&post.ID, &post.Title, &post.Preview, &post.Date, &post.Content, &post.Author.ID, &post.Author.Username, &post.Author.Name, &post.Author.Avatar)
		if err != nil {
			return posts, err
		}
	}
